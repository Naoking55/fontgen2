...
変更履歴:
- v1.82.12 (2025-11-06): 重大なバグ修正 - 動的境界検出が実際に動作するように 🐛
  * extract_single_partの未定義変数 char_desc を修正
  * これにより動的境界検出が NameError で失敗していた問題を解決
  * 今度こそ本当に動的境界検出が機能します！
- v1.82.11 (2025-11-06): デバッグ強化版 - 動的境界検出とクリップボード 🔍
  * 動的境界検出に詳細なログ出力を追加
    - 検出の開始/成功/失敗を明示
    - 元のratio値と新しいratio値を比較表示
    - エラーの詳細をトレースバック表示
  * クリップボード操作に詳細なログ出力を追加
    - 設定時と取得時のprojectインスタンスを表示
    - クリップボード状態を各ステップで表示
  * これにより、ユーザーは何が起きているか確認可能
...
# ===== GlyphEditorクラスの続き =====
    ...
    def _paste(self) -> None:
        ...
        print(f"[PASTE] _paste called in GlyphEditor")
        print(f"[PASTE] self.project: {self.project}")
        print(f"[PASTE] self.project.clipboard: {self.project.clipboard}")
        if self.project.clipboard:
            print(f"[PASTE] clipboard size: {self.project.clipboard.size}")
        else:
            print(f"[PASTE] clipboard is None or empty")
...
class FontEditorApp(tk.Tk):
    ...
    def _insert_part_to_active_editor(self, part_image: Image.Image, part_name: str, offset: Tuple[float, float] = (0.0, 0.0)) -> None:
        ...
        try:
            print(f"[CLIPBOARD] _insert_part_to_active_editor開始: part_name={part_name}")
            print(f"[CLIPBOARD] part_image size: {part_image.size}")

            # 変形ダイアログを表示
            dialog = PartTransformDialog(self, part_image, part_name)
            self.wait_window(dialog)

            print(f"[CLIPBOARD] ダイアログ終了, dialog.result: {dialog.result}")

            if dialog.result:
                # 変形された画像をクリップボードにコピー
                transformed_image = dialog.result
                print(f"[CLIPBOARD] transformed_image size: {transformed_image.size}")
                print(f"[CLIPBOARD] self.project: {self.project}")
                print(f"[CLIPBOARD] self.project.clipboard (設定前): {self.project.clipboard}")

                self.project.clipboard = transformed_image

                print(f"[CLIPBOARD] self.project.clipboard (設定後): {self.project.clipboard}")
                print(f"[CLIPBOARD] Clipboard size: {self.project.clipboard.size}")
                messagebox.showinfo("クリップボードにコピー",
                    f"偏旁「{part_name}」を変形してクリップボードにコピーしました。\n"
                    "文字編集ウィンドウで「📄 貼付」ボタンまたは Ctrl+V / ⌘V で貼り付けてください。")
            else:
                print(f"[CLIPBOARD] dialog.result が None (キャンセルされた)")
        except Exception as e:
            print(f"[CLIPBOARD] エラー発生: {e}")
            import traceback
            traceback.print_exc()
...
def extract_single_part(font_path, part_name, part_info, output_path, noise_removal=True):
    ...
    try:
        ...
        if Config.DYNAMIC_BOUNDARY_DETECTION:
            try:
                print(f"[DYNAMIC-DETECT] 動的境界検出開始: {part_name} ({used_char}), split_type={split_type}, 元ratio={ratio}")
                ...
                if split_type in ["left", "right"]:
                    ...
                    direction = None
                    print(f"[DYNAMIC-DETECT] split_type '{split_type}' は動的検出非対応（固定ratioを使用）")
                ...
                if direction:
                    ...
                    if candidates_dynamic:
                        ...
                        print(f"[DYNAMIC-DETECT] 検出成功: 新ratio={used_ratio:.3f} (元={ratio:.3f}), スコア={score:.3f}")
                    else:
                        print(f"[DYNAMIC-DETECT] 候補が見つからず、固定ratioを使用")
            except Exception as e:
                # 動的検出に失敗した場合は固定ratioを使用（無視して続行）
                print(f"[DYNAMIC-DETECT] エラー発生: {e}, 固定ratioを使用")
                import traceback
                traceback.print_exc()
