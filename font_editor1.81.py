# --- BEGIN AUTO-IMPORTS (tk/PIL/typing) ---
...
from typing import Optional, List, Tuple, Set, Dict, Any, Callable  # [FIX] for type hints
from PIL import Image, ImageFont, ImageDraw, ImageOps, ImageTk  # [FIX] consolidate PIL imports at top for type hints
from types import MethodType  # [FIX] used in GlyphEditor patch
...
# ===== PATCH-A: keep parts editor alive + persist editor state =====
...
class PartsPreviewWindow(tk.Toplevel):
    ...
    def _setup_ui(self):
        ...
        self.bind('<Control-y>', lambda e: self._redo())
        self.bind('<Control-Shift-Z>', lambda e: self._redo())
    def _copy_to_main(self):
        global _COPY_TO_MAIN_CALLBACK
        if _COPY_TO_MAIN_CALLBACK and self.current_image is not None:
            try:
                _COPY_TO_MAIN_CALLBACK(self.current_image.copy(), 1.0, (0,0))
                try:
                    self.bell()
                except Exception:
                    pass
            except Exception as e:
                print('copy_to_main error:', e)
        else:
            print('no copy callback or image')
...
# [INTEGRATED] expose setter on GUI class if present
try:
    def _ipe_set_copy_callback(self, cb):
        set_copy_callback(cb)
    PartsExtractorGUI.set_copy_callback = _ipe_set_copy_callback
except Exception:
    pass
...
# [INTEGRATED] add setter methods to PartsExtractorGUI
try:
    def _ipe_set_font_path(self, path):
        self.font_path = path
        try:
            self.font_label.config(text=str(path))
        except Exception:
            pass
    def _ipe_set_output_dir(self, d):
        self.output_dir = d
        try:
            self.output_label.config(text=str(d))
        except Exception:
            pass
    PartsExtractorGUI.set_font_path = _ipe_set_font_path
    PartsExtractorGUI.set_output_dir = _ipe_set_output_dir
except Exception:
    pass
...
def remove_noise(img, min_size=2048):
    """ノイズ除去（孤立した小さなピクセル塊を削除）"""
    if img is None:
        return None
...
# [INTEGRATED] entry removed


# ---- integrated helpers & aliases ----
try:
    IntegratedPartsExtractorGUI
except NameError:
    try:
        IntegratedPartsExtractorGUI = PartsExtractorGUI
    except Exception:
...
try:
    if not hasattr(PartsExtractorGUI, "set_copy_callback"):
        def _ipe_set_copy_callback(self, cb):
            try: self._copy_callback = cb
            except Exception: pass
        PartsExtractorGUI.set_copy_callback = _ipe_set_copy_callback
except Exception:
    pass
...
try:
    if not hasattr(PartsExtractorGUI, "set_font_path"):
        def _ipe_set_font_path(self, path):
            self.font_path = path
            try: self.font_label.config(text=str(path))
            except Exception: pass
        PartsExtractorGUI.set_font_path = _ipe_set_font_path
except Exception:
    pass
...
try:
    if not hasattr(PartsExtractorGUI, "set_output_dir"):
        def _ipe_set_output_dir(self, d):
            self.output_dir = d
            try: self.output_label.config(text=str(d))
            except Exception: pass
        PartsExtractorGUI.set_output_dir = _ipe_set_output_dir
except Exception:
    pass
...
try:
    if not hasattr(PartsExtractorGUI, "get_output_dir"):
        def _ipe_get_output_dir(self):
            return getattr(self, "output_dir", None)
        PartsExtractorGUI.get_output_dir = _ipe_get_output_dir
except Exception:
    pass
