...
変更履歴:
- v1.82.16 (2025-11-07): 偏旁パレット貼り付けをクリップボード経由に変更 📋✨
  * 偏旁パレットからクリップボード経由で貼り付けられるように修正
    - 「貼付」ボタンをクリックするとクリップボードにコピー
    - 文字編集ウィンドウで「📄 貼付」ボタンまたはCtrl+V/⌘Vで貼り付け
    - RGBA画像を適切にグレースケールに変換
  * ユーザーフィードバックに基づく改善
    - より柔軟な貼り付けワークフローに対応
    - 貼り付け前に位置を調整可能
  * 文字挿入機能は正常に実装されています
...
class FontEditorApp(tk.Tk):
    ...
    def _insert_part_to_active_editor(self, part_image: Image.Image, part_name: str, offset: Tuple[float, float] = (0.0, 0.0)) -> None:
        """[FIX v1.82.16] 偏旁をクリップボードに格納して貼り付け可能にする"""
        try:
            ...
            if part_image.mode == 'RGBA':
                # 白背景にアルファ合成
                bg = Image.new('L', part_image.size, 255)
                alpha = part_image.split()[3]
                # アルファ値を使って合成
                img_l = part_image.convert('L')
                # 白背景に対してアルファブレンド
                result = Image.composite(img_l, bg, alpha)
                part_image = result
            elif part_image.mode != 'L':
                part_image = part_image.convert('L')

            # クリップボードにコピー
            self.project.clipboard = part_image.copy()
            self._log(f"偏旁をクリップボードにコピー: {part_name} (サイズ: {part_image.size})")

            messagebox.showinfo(
                "クリップボードにコピー",
                f"偏旁「{part_name}」をクリップボードにコピーしました。\n\n"
                "文字編集ウィンドウで以下の方法で貼り付けてください：\n"
                "• 「📄 貼付」ボタンをクリック\n"
                "• キーボードショートカット: Ctrl+V (Windows/Linux) または ⌘V (Mac)"
            )
        except Exception as e:
