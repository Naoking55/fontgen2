...
def _open_parts_editor(self) -> None:
    """偏旁エディタ（統合版）を起動して、同一プロジェクトのフォント・出力先・コピペ連携をセットする"""
    ...
    try:
        ...
        gui = PartsExtractorGUI(top)  # 統合済みの偏旁エディタGUI
    font_path = self.project.font_path
    out_dir = self._get_parts_output_dir() if hasattr(self, '_get_parts_output_dir') else os.path.join(os.path.dirname(font_path), 'parts')
    try:
        os.makedirs(out_dir, exist_ok=True)
    except Exception:
        pass
    try:
        if hasattr(gui, 'set_font_path'): gui.set_font_path(font_path)
        if hasattr(gui, 'set_output_dir'): gui.set_output_dir(out_dir)
        if hasattr(gui, 'set_render_size'):
            # 2048 を明示
            size = getattr(Config, 'CANVAS_SIZE', 2048)
            try: gui.set_render_size(size)
    ...
    def _import_back() -> None:
        try:
            # 取り込み→パレット前面
            if hasattr(self, '_import_parts_from_folder'):
                self._import_parts_from_folder(out_dir)
            if hasattr(self, '_open_parts_palette'):
                self._open_parts_palette()
            messagebox.showinfo('取り込み完了', '偏旁を本体へ取り込みました')
        except Exception as e:
...
def _open_parts_palette(self) -> None:
    """偏旁パレットウィンドウを開く。パーツが無ければ抽出ツールを案内。"""
    try:
...
# ===== [FIX] Ensure palette/editor methods exist & are bound BEFORE main =====
def _fe_open_parts_palette(self) -> None:
    try:
        if getattr(self, 'parts_palette', None) and self.parts_palette.winfo_exists():
            self.parts_palette.lift()
            return
    ...
    if not getattr(self.project, 'parts', None):
        ...
        return
    try:
        self.parts_palette = PartsPalette(self, self.project, getattr(self, '_insert_part_to_active_editor', lambda *a, **k: None))
        self.parts_palette.lift()
    except Exception:
        # Fallback: open editor
        try: self._open_parts_editor()
        except Exception: pass

def _fe_open_parts_editor(self) -> None:
    if not getattr(self.project, 'font_path', None):
        messagebox.showwarning('警告', 'まずフォントを読み込んでください')
        return
    top = tk.Toplevel(self); top.title('偏旁エディタ')
    try:
        gui = IntegratedPartsExtractorGUI(top)
    except NameError:
        gui = PartsExtractorGUI(top)
    font_path = self.project.font_path
    out_dir = os.path.join(getattr(self.project, '_project_dir', os.path.dirname(font_path)), 'parts')
...
# Bind to class
try:
    FontEditorApp  # type: ignore
    FontEditorApp._open_parts_palette = _fe_open_parts_palette  # type: ignore
    FontEditorApp._open_parts_editor  = _fe_open_parts_editor   # type: ignore
except Exception:
    pass
...
# ===== [APPEND: save/confirm bindings] =====
try:
    FontEditorApp
except NameError:
    pass
else:
    if not hasattr(FontEditorApp, "_confirm_unsaved_changes"):
        def _confirm_unsaved_changes(self) -> bool:
            from tkinter import messagebox
            if not getattr(self, "project", None) or not getattr(self.project, "dirty", False):
                return True
            ans = messagebox.askyesnocancel(
                "未保存の変更",
                "編集中の変更があります。プロジェクトとして保存しますか？\n\n"
            ...
            if ans:
                return self._save_project_dialog()
            return True
        FontEditorApp._confirm_unsaved_changes = _confirm_unsaved_changes

    if not hasattr(FontEditorApp, "_save_project_dialog"):
        def _save_project_dialog(self) -> bool:
            from tkinter import filedialog, messagebox
            try:
                if hasattr(self, "_save_project"):
            ...
            try:
                path = filedialog.asksaveasfilename(
                    title="プロジェクトを保存",
                    defaultextension=".fproj",
                    filetypes=[("Font Project", "*.fproj"), ("All Files", "*.*")]
                )
                if not path:
                    return False
                if hasattr(self, "project") and hasattr(self.project, "save"):
                    ...
                    return True
                messagebox.showwarning("保存", "保存APIが見つかりません（project.save が未実装）。")
            except Exception as e:
                messagebox.showerror("保存エラー", f"保存に失敗しました:\n{e}")
            return False
        FontEditorApp._save_project_dialog = _save_project_dialog
# ===== [APPEND END] =====

# [FIX-BIND] Ensure methods are bound to FontEditorApp
try:
    FontEditorApp  # type: ignore
    if '_open_parts_palette' in globals():
        FontEditorApp._open_parts_palette = _open_parts_palette  # type: ignore
    if '_open_parts_editor' in globals():
        FontEditorApp._open_parts_editor = _open_parts_editor  # type: ignore
    if '_insert_part_to_active_editor' in globals():
        FontEditorApp._insert_part_to_active_editor = _insert_part_to_active_editor  # type: ignore
except Exception:
    pass
...
# [INTEGRATED] main entry removed

# [INTEGRATED] alias for main editor
IntegratedPartsExtractorGUI = PartsExtractorGUI

# ===== [INTEGRATED-PARTS] END =====
