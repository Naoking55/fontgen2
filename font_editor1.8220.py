...
変更履歴:
- v1.82.20 (2025-11-07): PartsPreviewWindow の完全修正 🔧🎨
  * ズーム機能の修正:
    - 拡大時に画像が見えなくなる問題を解消
    - プレビューキャンバスに水平・垂直スクロールバーを追加
    - 画像サイズに応じて自動的にスクロール領域を設定
    - 小さい画像は中央配置、大きい画像はスクロール可能に
  * 消しゴム機能の完全修正:
    - ドラッグで滑らかに動作するように座標変換を修正
    - canvasx/canvasy メソッドでスクロール位置を正しく取得
    - キャンバス座標とウィンドウ座標の変換を適切に処理
    - マウスカーソル位置の表示が正確に追従するように修正
  * 再抽出機能のバグ修正:
    - current_split_type 変数の未初期化エラーを修正
    - __init__ メソッドで 'left' に初期化
  * すべてのキャンバス編集機能が正常に動作
...
def extract_all_parts(font_path, output_dir, progress_callback=None, log_callback=None):
    ...
    log("=" * 70)
    log("🔍 偏旁抽出ツール v2.9.5-DEBUG (超詳細ログ版) 🐛")
    log("=" * 70)
    log(f"フォント: {font_path}")
    log(f"出力先: {output_dir}")
    log(f"動的境界検出: {'✅ 有効 (各パーツで[DEBUG]ログが表示されます)' if Config.DYNAMIC_BOUNDARY_DETECTION else '❌ 無効'}")
    if Config.DYNAMIC_BOUNDARY_DETECTION:
        ...
        log("  ※ 各パーツ処理時に [DEBUG] タグ付きログが表示されます")
    log("=" * 70)
    log("")
...
class PartsPreviewWindow(tk.Toplevel):
    ...
    def __init__(self, parent, parts_dir, font_path):
        ...
        self.current_split_type = 'left'  # [FIX] v1.82.20: 初期化を追加
        self.photo_cache = {}
    ...
    def _setup_ui(self):
        ...
        # プレビューキャンバス（スクロールバー付き）  # [FIX] v1.82.20: スクロール対応
        canvas_container = ttk.Frame(preview_frame)
        canvas_container.pack(fill=tk.BOTH, expand=True)

        self.preview_canvas = tk.Canvas(canvas_container, width=400, height=350, bg="white", relief="sunken", borderwidth=2)

        # スクロールバー
        h_scrollbar = ttk.Scrollbar(canvas_container, orient=tk.HORIZONTAL, command=self.preview_canvas.xview)
        v_scrollbar = ttk.Scrollbar(canvas_container, orient=tk.VERTICAL, command=self.preview_canvas.yview)
    ...
    def _update_preview(self):
        """プレビュー更新"""  # [FIX] v1.82.20: スクロール対応に変更
        ...
        # 画像が小さい場合は中央配置、大きい場合はスクロール領域を設定
    ...
    def _on_canvas_motion(self, event):
        """マウス移動時の処理"""  # [FIX] v1.82.20: スクロール対応
        if self.eraser_mode:
            # ウィンドウ座標をキャンバス座標に変換
    ...
    def _canvas_to_image_coords(self, canvas_x, canvas_y):
        """キャンバス座標を画像座標に変換"""  # [FIX] v1.82.20: スクロール対応
        # スクロール位置を取得
        scroll_x = self.preview_canvas.canvasx(canvas_x)
        scroll_y = self.preview_canvas.canvasy(canvas_y)

        # 画像のオフセットを引く
        img_display_x = scroll_x - self.image_offset_x
        img_display_y = scroll_y - self.image_offset_y

        # 表示スケールで割って元の画像座標に変換
        img_x = int(img_display_x / self.preview_scale)
        img_y = int(img_display_y / self.preview_scale)
    ...
    def _interpolate_erase(self, x1, y1, x2, y2):
        ...
        # 画像座標をキャンバス座標に変換
        canvas_x = int(x2 * self.preview_scale + self.image_offset_x)
        canvas_y = int(y2 * self.preview_scale + self.image_offset_y)
        self._update_eraser_cursor_position(canvas_x, canvas_y)
...
class PartsExtractorGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("🔍 偏旁抽出ツール v2.9.5-DEBUG (2025-11-07) - 動的境界検出+超詳細ログ 🐛")
    ...
    def _setup_ui(self):
        ...
        self._log("🔍 偏旁抽出ツール v2.9.5-DEBUG (2025-11-07) 🐛")
        self._log("=" * 70)
        self._log("【本バージョンの特徴】")
        self._log("  🔍 超詳細デバッグログ: 各パーツの処理状況を完全可視化")
        self._log("  ✅ 動的境界検出: 画像解析で最適な分割位置を自動検出")
        self._log("  ✅ 高精度抽出: 接触文字でも境界を正確に判定")
        self._log("  📊 実行状況表示: [DEBUG]タグで各ステップの詳細を表示")
        self._log(f"  ⚙️ 設定: 動的検出 {'✅ 有効' if Config.DYNAMIC_BOUNDARY_DETECTION else '❌ 無効'}")
        self._log("=" * 70)
        self._log("【重要】以下のログに[DEBUG]タグが表示されるはずです")
